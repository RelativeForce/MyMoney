...
const DUPLICATE_INCOME_ERROR =
   'Duplicate income: Two incomes with the same name cannot exist on the same day';
const FAILED_IMPORT_ERROR = 'Failed to import income';

export class IncomeImportDataProvider
   implements IImportDataProvider<IncomeProperty>
{
   ...
   public markAsUnique(rows: Row[]): void {
      rows
         .filter((row) => row.error === DUPLICATE_INCOME_ERROR)
   ...
   public setupKeyExtractor(
      headings: BaseHeading<IncomeProperty>[]
   ): (row: Row) => string {
      const dateIndex = headings.findIndex(
         (h) => h.property === IncomeProperty.date
      );
      const nameIndex = headings.findIndex(
         (h) => h.property === IncomeProperty.name
      );

      return (row: Row): string => {
         const date = IncomeHeading.formatWithTransactionProperty(
            IncomeProperty.date,
            row.data[dateIndex]
         );
         const name = IncomeHeading.formatWithTransactionProperty(
            IncomeProperty.name,
            row.data[nameIndex]
         );
   ...
   public submit(
      ...
      const amountIndex = headings.findIndex(
         (h) => h.property === IncomeProperty.amount
      );
      const notesIndex = headings.findIndex(
         (h) => h.property === IncomeProperty.notes
      );

      function toTransaction(
         row: Row
      ): { income: IIncomeDto; row: Row } | null {
         const date: string | null =
            IncomeHeading.formatWithTransactionProperty(
               IncomeProperty.date,
               row.data[dateIndex]
            );
         const amount: number | null =
            IncomeHeading.formatWithTransactionProperty(
               IncomeProperty.amount,
   ...
   public columnErrorMessage(
      ...
      const nameFieldCount = headings.filter(
         (h) => h.property === IncomeProperty.name
      ).length;
   ...
   private isComplete(
      transactionRows: { income: IIncomeDto; row: Row }[]
   ): boolean {
