...
export const sessionSlice = createSlice({
   ...
   extraReducers(builder: ActionReducerMapBuilder<ISessionState>) {
      builder
         ...
         .addCase(
            login.fulfilled,
            (state: ISessionState, action: { payload: ILoginResultDto }) => {
...
export const selectSessionState = (state: IAppState): ISessionState =>
   state.session;
export const selectCurrentSession = (state: IAppState): ISessionModel | null =>
   selectSessionState(state).currentSession;
export const selectCurrentSessionToken = (state: IAppState): string | null =>
   selectCurrentSession(state)?.token ?? null;
export const selectCurrentUser = (state: IAppState): IUserDto | null =>
   selectCurrentUserState(state).data;
export const selectCurrentUserState = (
   state: IAppState
): IAsyncState<IUserDto | null> => selectSessionState(state).currentUser;
