...
export class AuthenticationApi {
   ...
   public register(newUserData: IRegisterDto): Observable<ILoginResultDto> {
      return this.api.post<IRegisterDto, ILoginResultDto>('/Authentication/Register', newUserData).pipe(first());
   }

   public forgotPassword(email: IForgotPasswordDto): Observable<void> {
      return this.api.post<IForgotPasswordDto, void>('/Authentication/ForgotPassword', email).pipe(first());
   }

   public resetPassword(newPassword: IPasswordDto, userToken: string): Observable<IBasicResultDto> {
      return this.api.post<IPasswordDto, IBasicResultDto>('/User/ChangePassword', newPassword, userToken).pipe(first());
   }
}
