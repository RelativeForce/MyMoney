...
export class UserApi {
   ...
   public updateCurrentUserDetails(user: IUserDto): Observable<IBasicResultDto> {
      return this.api.post<IUserDto, IBasicResultDto>('/User/UpdateSignedInUser', user).pipe(first());
   }

   public changePassword(password: IPasswordDto): Observable<IBasicResultDto> {
      return this.api.post<IPasswordDto, IBasicResultDto>('/User/ChangePassword', password).pipe(first());
   }
}
